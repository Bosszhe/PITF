...
class TransFuserAgent(autonomous_agent.AutonomousAgent):
	...
	def __init__(self, path_to_conf_file, config: dict = None):
		self.cfg = config
	...
	def setup(self, path_to_conf_file):
		...
		self.input_buffer = {'rgb': deque(), 'rgb_left': deque(), 'rgb_right': deque(), 
							'rgb_rear': deque(), 'lidar': deque(), 'raw_lidar': deque(), 'gps': deque(), 'thetas': deque()}

		self.config = GlobalConfig()
		self.config.pc_bb = self.cfg['pc_bb']
	...
	def run_step(self, input_data, timestamp):
		...
		if self.step < self.config.seq_len:
			...
			self.input_buffer['raw_lidar'].append(tick_data['raw_lidar'])
		...
		self.input_buffer['lidar'].append(tick_data['lidar'])
		self.input_buffer['raw_lidar'].popleft()
		self.input_buffer['raw_lidar'].append(tick_data['raw_lidar'])
		...
		if(self.step  % 2 == 0 or self.step <= 4):
			if 'pc_bb' not in self.cfg or self.cfg['pc_bb'] == 'bev':
				...
				# end for
				# network forwarding
				self.pred_wp = self.net(
					self.input_buffer['rgb'] + self.input_buffer['rgb_left'] + \
					self.input_buffer['rgb_right'] + self.input_buffer['rgb_rear'],
					self.lidar_processed, 
					target_point, 
					gt_velocity
				)
			elif self.cfg['pc_bb'] == 'pp':
				for i, raw_lidar_pc in enumerate(self.input_buffer['raw_lidar']):
					curr_theta = self.input_buffer['thetas'][i]
					curr_x, curr_y = self.input_buffer['gps'][i]
					raw_lidar = torch.from_numpy(raw_lidar_pc).to('cuda', dtype=torch.float32)
					self.raw_lidar = list()
					self.raw_lidar.append([raw_lidar])
				# end for
				# network forwarding
				self.pred_wp = self.net(
					self.input_buffer['rgb'] + self.input_buffer['rgb_left'] + \
				...
				raise NotImplementedError(self.cfg['pc_bb'])
